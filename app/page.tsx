...
import { Search, Heart, Shield, Globe, CheckCircle, Calendar, Users, Award, ArrowRight, Sparkles, MapPin, List, Map, X as XIcon } from 'lucide-react';
...
export default function Home() {
    ...
    const handleBook = (doctorId: string, slot: string) => {
        ...
        const appointment: Appointment = {
            ...
            doctorId,
            patientId: user?.id,
            patientName: user?.name || "Paciente marketplace",
            dateTime: slot,
            status: 'pending',
            mode: 'online',
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col lg:flex-row items-center justify-between gap-12 relative z-10">
                    <div className="lg:w-1/2">
                        ...
                        <div className="flex flex-wrap gap-4">
                            <button className="bg-white text-gray-900 px-6 py-4 rounded-2xl font-bold flex items-center gap-3 hover:bg-gray-100 transition shadow-lg" onClick={() => window.open('https://apps.apple.com', '_blank')}>
                                <AppleIcon className="w-6 h-6" /> App Store
                            </button>
                            <button className="bg-white/10 backdrop-blur-sm text-white px-6 py-4 rounded-2xl font-bold flex items-center gap-3 hover:bg-white/20 transition border border-white/20" onClick={() => window.open('https://play.google.com', '_blank')}>
